...
class PopoverElement extends Component {
  static propTypes = {
    ...
    arrowOffsetPercentage: PropTypes.shape({
      x: PropTypes.number,
      y: PropTypes.number,
    })
  };
  ...
  render() {
    ...
    if (this.state.arrowStyle) {
      if (this.props.arrowOffsetPercentage && this.props.arrowOffsetPercentage.x) {
        arrowStyles.push({
          left: this.state.arrowStyle.left + (this.state.arrowStyle.left * this.props.arrowOffsetPercentage.x / 100),
        });
      }
      if (this.props.arrowOffsetPercentage && this.props.arrowOffsetPercentage.y) {
        arrowStyles.push({
          bottom: this.state.arrowStyle.left + (this.state.arrowStyle.left * this.props.arrowOffsetPercentage.y / 100),
        });
      }
    }
    return (
